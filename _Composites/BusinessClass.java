...
public class BusinessClass extends AirplaneComponent {
    int lastSeat = 0;
    ...
    @Override
    public String info() {
        String accum = "Business class -";
        for(AirplaneComponent passenger : passengers) {
            accum = accum + passenger.info();
            if(passenger.getWeight() > 35)
                accum = accum + "\nПЕРЕГРУЗ! ДОПЛАЧЕНО!" ;
    ...
    public void passengerBoarding(String name, AirClass ticket, double weight, int SeatNum) {
        if(lastSeat == passengers.size()){
            System.out.print("BusinessClass is full");
            return;
        }
        if(weight > 35){
            System.out.print("weight is bigger than 35 but it is ok");
        }
